...
const Favorites = () => {
  ...
  const removeFromFavorites = async (recipeId) => {
    try {
      ...
      await axios.delete(`${process.env.REACT_APP_API_URL}/favorites/removeFromFavorites/${cookies.user_id}/${recipeId}`);
      ...
      try {
        if (cookies.user_id) {
          const response = await axios.get(
            `${process.env.REACT_APP_API_URL}/recipes/favoriteRecipes/${cookies.user_id}`
          );
