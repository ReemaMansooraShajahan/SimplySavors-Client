...
// };
  const fetchUserProfile = async (token) => {
    ...
    try {
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/auth/user/profile`, config);
  ...
  const fetchUserSettings = async (userId, token) => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/settings/getSettings/${userId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
