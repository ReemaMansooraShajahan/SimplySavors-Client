...
export default function Settings() {
  ...
  const updateSettingsOnServer = async (newSettings) => {
    try {
      ...
      const response = await axios.put(
        `${process.env.REACT_APP_API_URL}/settings/updateSettings/${cookies.user_id}`,
  ...
  const fetchUserSettings = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/settings/getSettings/${cookies.user_id}`, {
        headers: { Authorization: `Bearer ${cookies['access_token']}` },
      });
