...
const UserProfile = () => {
  ...
  useEffect(() => {
    const fetchUserProfile = async (token , userID) => {
      ...
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_URL}/auth/user/profile/${userID}`, config);
    ...
    const fetchUserPostedRecipes = async (userID) => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_URL}/recipes/postedRecipes/${userID}`);
  ...
  const deleteRecipe = async (recipeID) => {
    try {
      ...
      const response = await axios.delete(`${process.env.REACT_APP_API_URL}/recipes/${recipeID}`, config);

      if (response.status === 204) {
        // Recipe deleted successfully, update UI
        alert('Recipe deleted successfully');
        // Refetch user posted recipes after deletion
        const responsePostedRecipes = await axios.get(`${process.env.REACT_APP_API_URL}/recipes/postedRecipes/${userProfile._id}`);
